...
class MyListing {
	...
	function load() {
		if ($res = mysqli_query($this->c['db'],$this->prepare_query())) {
			$this->_count = 0;
			$class;
			$this->list = [];
			if (isset($this->definition['my_class']) && ($this->definition['my_class'] != '')) {
			...
			while ($values = mysqli_fetch_assoc($res)) {
				...
				foreach ($this->definition['counts'] as $key => $val) {
					$value->add_property(str_replace('.', '_', $key));
				}
	...
	protected function prepare_query() {
		...
		if (!$query) {
			...
			foreach ($this->definition['filters'] as $key => $value) {
				...
				$query_suffix .= ($query_suffix ? ' AND ' : ' WHERE ') . $key . ' ' . $condition . (isset($value) ? " '" . mysqli_real_escape_string($this->c['db'], $value) . "'" : '' ) . '';
			}
			...
			foreach ($this->definition['counts'] as $key => $value) {
				...
				$counts = ($counts ? $counts : ' ') . " LEFT JOIN (SELECT $value, COUNT($value) AS cnt FROM $column GROUP BY $value) c$index ON m.$value = c$index.$value";
				$index++;
			}
